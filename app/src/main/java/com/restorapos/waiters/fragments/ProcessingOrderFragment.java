...
import com.restorapos.waiters.databinding.FragmentProcessingOrderBinding;
...
public class ProcessingOrderFragment extends Fragment implements ViewInterface {
    private FragmentProcessingOrderBinding binding;
    ...
    private void customFilterList(String value) {
        ...
        if (value != null && !value.isEmpty()) {
            ...
            if(items.size()!= 0){
                ...
                pendingOrderAdapter = new PendingOrderAdapter(getActivity().getApplicationContext(), newList,ProcessingOrderFragment.this::viewOrder);
                binding.processingRecycler.setAdapter(pendingOrderAdapter);

            }else {
                pendingOrderAdapter = new PendingOrderAdapter(getActivity().getApplicationContext(), items,ProcessingOrderFragment.this::viewOrder);
                binding.processingRecycler.setAdapter(pendingOrderAdapter);
            }
        }
    }
